...
def main():
    ...
    argc = len(args)
    if argc != 2:
        print("error: please input a specqq file!")
        sys.exit()
    allspec = []
    ...
    fw_x = open("angspec_x.dat",mode = "w")
    fw_x.write("#")
    fw_x.write(" omega, I_shita")
    fw_x.write(" ( sita(degree) = ")
    fw_y = open("angspec_y.dat",mode = "w")
    fw_y.write("#")
    fw_y.write(" omega, ,I_shita")
    fw_y.write(" ( sita(degree) = ")
    fw_av = open("angspec_av.dat",mode = "w")
    fw_av.write("#")
...
class Spec():
    def __init__(self,energy,mm,mz,mp,zm,zz,zp,pm,pz,pp):
        self.energy = energy
        self.mm = mm
        self.mz = mz
        self.mp = mp
    ...
    def specchange_x(self,ang,fw):
        ...
        newspec = math.cos(rad)**2 * self.zz + math.sin(rad)**2 * (self.pp + self.mm - 2 * self.pm) / 2 - math.sqrt(2) * (self.zp - self.mz) * math.sin(rad) * math.cos(rad)
        fw.write(str("{:>11}".format("{:.6f}".format(newspec))))
        self.angspec_x.append(newspec)
        return newspec
    def specchange_y(self,ang,fw):
        rad = math.radians(ang)
        newspec = math.cos(rad)**2 * self.zz + math.sin(rad)**2 * (self.pp + self.mm + 2 * self.pm) / 2 
        fw.write(str("{:>11}".format("{:.6f}".format(newspec))))
