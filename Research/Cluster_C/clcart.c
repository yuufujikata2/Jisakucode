...
*/
  ...
  printf("\nNo of basis atoms: ") ; fscanf(fp,"%d",&nbasisatoms) ;
  ...
  for ( i=0 ; i<nbasisatoms ; i++ ) {
    ...
    fscanf(fp,"%s%lf%lf%lf", type[i], &cart[i][0], &cart[i][1], &cart[i][2] ) ;
    printf("%s %lf %lf %lf\n", type[i], cart[i][0], cart[i][1], cart[i][2] ) ;
  }

  printf("lattice constant in Angs? ") ; fscanf(fp,"%lf",&latticeconstant) ;
  printf("\nradius of ellipsoid in Angs? ") ; fscanf(fp,"%lf", &radius2 ) ;
  cutoff = radius2 ;
  radius2 *= radius2 ;
  printf("\ndepth of ellipsoid in Angs? ") ; fscanf(fp,"%lf", &depth2 ) ;
  if ( depth2 > cutoff ) cutoff = depth2 ;
  depth2 *= depth2 ;
  printf("\nzsurf in Angs? ") ; fscanf(fp,"%lf", &zsurf ) ;
  ...
  printf("%d %d %d",i0max,i1max,i2max);

  printf("\nCoordinates\n") ;
  for ( i0 = -i0max ; i0 <= i0max ; i0++ ) {
    for ( i1 = -i1max ; i1 <= i1max ; i1++ ) {
      for ( i2 = -i2max ; i2 <= i2max ; i2++ ) {
