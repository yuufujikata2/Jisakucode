...
#    mlab.show()
    ...
    for e_num in range(1,2):
        ...
        """
        count = 0
        fr_lam = open("lambda_orig.dat",mode="r")
        lam_lines = fr_lam.readlines()
            ...
            for m1 in range (-l1,l1+1):
                for n1 in range(node):
                    for l2 in range(LMAX):
                        for m2 in range(-l2,l2+1):
                            for n2 in range(node):
                                lam_line = lam_lines[count].split()
                                lambda_mat[l1 **2 * node * LMAX**2 * node + (l1 + m1) * node * LMAX**2 * node + n1 * LMAX**2 * node + l2 **2 * node + (l2 + m2) * node + n2] = float(lam_line[1])
                                #if l1 == l2 and m1 == m2 :
