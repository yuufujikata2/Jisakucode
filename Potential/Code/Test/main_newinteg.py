...
def main():
    ...
    #V_ang = np.where(np.sqrt(xx * xx + yy * yy + zz * zz) < rofi[-1] , np.where(abs(xx) <= pot_region[0],np.where(abs(yy) <= pot_region[1],np.where(abs(zz) <= pot_region[2],  V - my_radial_interfunc(np.sqrt(xx **2 + yy **2 + zz **2)),- my_radial_interfunc(np.sqrt(xx **2 + yy **2 + zz **2))),- my_radial_interfunc(np.sqrt(xx **2 + yy **2 + zz **2))),- my_radial_interfunc(np.sqrt(xx **2 + yy **2 + zz **2))),0. )
    newintegral.jump_index(V_ang,x,y,z,-1,nr,rofi)
    sys.exit()
    """
