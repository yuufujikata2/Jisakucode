...
def main():
    ...
    #    region_t = np.where(dis < rofi[-1],1,0)
        ...
        g = np.zeros((node_open + node_close,LMAX,gridpx,gridpy,gridpz),dtype = np.float64)
        for n1 in range (node_open + node_close):
            for l1 in range (LMAX):
                my_radial_g_inter_func = interpolate.interp1d(rofi,all_basis[l1][n1].g[:nr])
                g[n1][l1] =  my_radial_g_inter_func(np.sqrt(xx**2 + yy **2 + zz **2))
            ...
            for n2 in range (node_open + node_close):
                for l1 in range (LMAX):
                    for l2 in range (LMAX):
                        ...
                        g_V_g = np.where(dis < rofi[-1],g[n1][l1] * V_ang * g[n2][l2],0.)
                        for m1 in range (-l1,l1+1):
                            for m2 in range (-l2,l2+1):
                                ...
                                fw_grid.write("{:>13.8f}".format(umat[l1][l2][m1][m2][n1][n2].real))
