...
def main():
    ...
    #    umat_t = np.zeros((LMAX,LMAX,2 * LMAX + 1,2 * LMAX + 1,node_open + node_close,node_open + node_close), dtype = np.complex64)
        ...
        V_ang_i = np.where(dis <= rofi[-1],np.where(abs(izz) <= pot_region[2],np.where(abs(iyy) <= pot_region[1],np.where(abs(ixx) <= pot_region[0] , -1 - my_radial_interfunc(np.sqrt(ixx **2 + iyy **2 + izz **2)), - my_radial_interfunc(np.sqrt(ixx **2 + iyy **2 + izz **2))), - my_radial_interfunc(np.sqrt(ixx **2 + iyy **2 + izz **2))), - my_radial_interfunc(np.sqrt(ixx **2 + iyy **2 + izz **2))),0.)
        #V_ang_i = np.where(ixx <= pot_region[0] and iyy <= pot_region[1] and izz <= pot_region[2], -1 - my_radial_interfunc(np.sqrt(ixx **2 + iyy **2 + izz **2)), - my_radial_interfunc(np.sqrt(ixx **2 + iyy **2 + izz **2)))
                    ...
                    for l2 in range (LMAX):
                        ...
                        for m1 in range (-l1,l1+1):
                            for m2 in range (-l2,l2+1):
                                ...
                                #umat[n1][n2][l1][l2][m1][m2] = np.average(V_ang_i)
                                fw_grid.write("{:>15.8f}".format(umat[n1][n2][l1][l2][m1][m2].real))
